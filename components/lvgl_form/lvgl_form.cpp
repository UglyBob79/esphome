...
FormPage *FormPage::active = nullptr;

void ActionInput::activate() { this->form_->on_submit(); }
...
lv_obj_t *FormPage::make_ticker_box(lv_obj_t *parent, int x, int y, lv_obj_t **label_out) {
...
void FormPage::build_ui() {
  ...
  for (auto &row : this->rows_) {
    ...
    } else if (row.type == RowType::TIME) {
      lv_obj_t *lbl = lv_label_create(this->page_obj_);
      lv_obj_set_pos(lbl, 20, row_y + 5);
      lv_label_set_text(lbl, row.time.label);
      if (this->font_)
        lv_obj_set_style_text_font(lbl, this->font_, (uint32_t)LV_PART_MAIN | (uint32_t)LV_STATE_DEFAULT);
      lv_obj_set_style_text_color(lbl, lv_color_make(0, 0, 0), (uint32_t)LV_PART_MAIN | (uint32_t)LV_STATE_DEFAULT);
...
void FormPage::on_page_load() {
  ESP_LOGD(LVGL_FORM_TAG, "on_page_load: %s (inputs: %d)", this->title_ ? this->title_ : "?", (int)this->inputs_.size());
  FormPage::active = this;
...
void FormPage::on_page_unload() {
  ...
  if (FormPage::active == this)
    FormPage::active = nullptr;
  this->page_active_ = false;
  this->editing_ = false;
}

void FormPage::on_confirm() {
  if (this->inputs_.empty())
    return;
